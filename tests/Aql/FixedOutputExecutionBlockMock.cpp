...
std::tuple<ExecutionState, SkipResult, SharedAqlItemBlockPtr> FixedOutputExecutionBlockMock::execute(
  ...
  if (_blockData.empty()) {
    std::tuple<ExecutionState, SkipResult, SharedAqlItemBlockPtr> res = {ExecutionState::DONE, skipped, nullptr};
    traceExecuteEnd(res);
    return res;
  }
  ...
  std::tuple<ExecutionState, SkipResult, SharedAqlItemBlockPtr> res = {state, skipped, block};
  traceExecuteEnd(res);
  return res;
}
