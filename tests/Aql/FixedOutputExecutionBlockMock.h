...
class FixedOutputExecutionBlockMock final : public arangodb::aql::ExecutionBlock {
 public:
  ...
  void setExecuteEnterHook(std::function<void(arangodb::aql::AqlCallStack const& stack)> hook);

 private:
  arangodb::aql::RegisterInfos _infos;
  std::deque<arangodb::aql::SharedAqlItemBlockPtr> _blockData;
  std::function<void(arangodb::aql::AqlCallStack const& stack)> _executeEnterHook;
};
