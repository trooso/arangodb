...
RegisterInfos MakeBaseInfos(RegisterCount numRegs, size_t subqueryDepth = 2) {
  ...
  return RegisterInfos(RegIdSet{0}, RegIdSet{0}, numRegs, numRegs, {}, regsToKeep);
}
...
TEST_F(SubqueryEndExecutorTest, count_shadow_rows_test) {
  ...
  FixedOutputExecutionBlockMock dependency{fakedQuery->rootEngine(), &inputNode,
                                           std::move(inputData)};
  MockTypedNode sqNode{fakedQuery->plan(), ExecutionNodeId{42}, ExecutionNode::SUBQUERY_END};
  ExecutionBlockImpl<SubqueryEndExecutor> testee{fakedQuery->rootEngine(), &sqNode,
                                                   MakeBaseInfos(2, 3), MakeExecutorInfos()};
