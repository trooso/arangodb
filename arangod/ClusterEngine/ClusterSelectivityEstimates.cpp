...
IndexEstMap ClusterSelectivityEstimates::get(bool allowUpdating, TransactionId tid) {
  ...
  if (allowUpdating) {
    ...
    do {
      ...
      if (!guard.try_lock()) {  // only one thread is allowed to fetch the estimates from the DB servers at any given time
        useExpired = true;
      } else {
...
void ClusterSelectivityEstimates::set(IndexEstMap estimates) {
  ...
  double ttl = defaultTtl;  // let selectivity estimates expire less often for system collections
  if (!_collection.name().empty() && _collection.name()[0] == '_') {
    ttl = systemCollectionTtl;
  }

  // finally update the cache
  std::atomic_store(&_data, std::make_shared<InternalData>(std::move(estimates),
                                                           TRI_microtime() + ttl));
}
